--GetStatisticsBySurveyID 3045

If Exists (select Name 
		   from sysobjects 
		   where  name = 'GetStatisticsBySurveyID' and
		        xtype = 'p')
Drop Procedure GetStatisticsBySurveyID 
go
Create PROCEDURE GetStatisticsBySurveyID @SurveyID int
as
...
select count(SMA.AnswerID) AnswersCount ,round((count(SMA.AnswerID)*1.0/@membersCount)*100,2) AnsPercentage,  A.AnswerID, A.ArName, A.EnName, Q.QuestionID, Q.ArTitle , Q.EnTitle  , SMA.AnswerText, SMA.QuestionID SurveyQuestionID
...
where Q.SurveyID = @SurveyID --and SM.IsSurveySubmited = 1
